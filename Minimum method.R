...
pdc_calc <- function(ID = 'MRN', Data.Date = Sys.Date(),
  ...
  pdc_calc_pid <- function(ID = 'MRN', patientID = 1,
    ...
    for (visit in visits) {
      ...
      date_med_id <- date_med_id %>%
        mutate(if_active_r = (threshold_start-7) <= visit_date & threshold_end > visit_date) %>%
        group_by(Drug) %>%
        mutate(if_active = any(if_active_r == TRUE)) %>%
        ungroup() %>%
        filter(if_active == TRUE) %>%
        distinct(ID,Drug,threshold_start,threshold_end,visit_date,if_active) %>%
        mutate(post_visit_7d = threshold_start > visit_date)
      
      date_med_id <- as.data.frame(date_med_id)
      
      # Create new objects for the interested time frame of the selected patient
      start <- visit
      end <- visit+Followed.Days
      ...
      if (nrow(date_med_id) > 0) {
        ...
        covered_days <- NULL
        covers <- data.frame(matrix(nrow = length(unique(date_med_id$Drug)),ncol = total_days))
        row.names(covers) <- c(unique(date_med_id$Drug))
        # sum_covers <- NULL
        ...
        df_pdc_id <- NULL
        df_pdc_id_visit <- NULL
        
        for (dr in 1:length(unique(date_med_id$Drug))) {
          df_pdc_id_drug <- data.frame(matrix(ncol = 5))
          colnames(df_pdc_id_drug) <- c("ID","drug","threshold_start","threshold_end","visit_date")
          df_pdc_id_drug$ID <- pid
          df_pdc_id_drug$drug <- unique(date_med_id$Drug)[dr]
          d <- unique(date_med_id$Drug)[dr]
          df_pdc_id_drug$threshold_start <- start
          df_pdc_id_drug$threshold_end <- end
          df_pdc_id_drug$visit_date <- visit
          df_pdc_id_drug$death_date <- death
          df_pdc_id_drug$hospitalization_days <- total_hosp_days
          df_pdc_id_drug$active_on_visit <- any(date_med_id[date_med_id$Drug==d,]$if_active == TRUE)
          df_pdc_id_drug$post_visit_7d <- all(date_med_id[date_med_id$Drug==d,]$post_visit_7d == TRUE)
          
          covered <- sum(covers[dr,]>0,na.rm = TRUE)
          df_pdc_id_drug$covered_days <- covered
          total <- sum(!is.na(covers[dr,]))
          df_pdc_id_drug$total_days <- total
          
          PDC <- covered/total
          df_pdc_id_drug$pdc <- PDC
          
          df_pdc_id_drug$gap <- sum(is.na(covers[dr,]))
          df_pdc_id_visit <- rbind(df_pdc_id_visit,df_pdc_id_drug)
        ...
        df_pdc_id <- data.frame(matrix(ncol = 5))
        colnames(df_pdc_id) <- c("ID","drug","threshold_start","threshold_end","visit_date")
        df_pdc_id$ID <- pid
        df_pdc_id$drug <- "None active med"
        ...
        df_pdc_id$active_on_visit <- NA
        df_pdc_id$post_visit_7d <- NA
        
        df_pdc_id$covered_days <- NA
  ...
  if(parallel == T){
    ...
    df_pdc <- foreach(x=patientID,.packages=c('lubridate', 'tidyverse'), 
                      ...
                      .combine = rbind ) %dopar% {
                        ...
                        pdc_calc_pid(data_dispensed = data_dispensed,
                                     patientID = x,
                                     ID = ID,
                                     Drug = Drug,
                                     Fill.Date = Fill.Date, 
    ...
    for (pid in patientID) {
      pdc_id <- pdc_calc_pid(data_dispensed = data_dispensed,
                             patientID = pid,
                             ID = ID,
                             Drug = Drug,
                             Fill.Date = Fill.Date, 
